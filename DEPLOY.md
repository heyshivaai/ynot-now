...
export default async function handler(req, res) {
  ...
  console.log(`[YNOT CRON] Starting weekly run ${runId} for ${runDate}`);

  // Run all 8 minds in parallel
  ...
  outcomes.forEach((outcome, i) => {
    ...
    if (outcome.status === 'fulfilled') {
      ...
      console.log(`[YNOT CRON] ${mind.name}: ${outcome.value.length} findings`);
    } else {
      results.errors.push({ mind: mind.id, error: outcome.reason?.message });
      console.error(`[YNOT CRON] ${mind.name} failed:`, outcome.reason?.message);
    }
  });
  ...
  // Store each finding with run metadata
  ...
  try {
    ...
    console.log(`[YNOT CRON] Stored ${rows.length} findings for run ${runId}`);
  } catch (err) {
    console.error('[YNOT CRON] Storage failed:', err.message);
    return res.status(500).json({ error: 'Storage failed', details: err.message });
  }

  // Update the trl_history table — track how each technology moves over time
  for (const f of results.findings) {
    const key = f.title.toLowerCase().replace(/[^a-z0-9]/g, '_').substring(0, 80);
    try {
      // Check if this technology already has history
      ...
      // Only record if TRL has changed or this is new
      ...
      // Non-fatal — history is nice to have
      console.warn(`[YNOT CRON] trl_history update skipped for ${key}:`, err.message);
    }
  }
