...
## Source Governance (`data/sources.json`)

**Edit `data/sources.json` to update source rules — no API code changes needed.**

This file is loaded at startup by `cron.js` and `cron-synthesise.js` and controls:

| Field | Purpose |
|-------|---------|
| `exclude_always` | Permanent domain blocklist — PwC, LinkedIn, consultancy reports, etc. Applied to every Tavily call AND `verifyRefs()`. Cannot be overridden by the feedback loop. |
| `authority_tlds` | TLDs that qualify a domain as authoritative for auto-promotion (`.gov`, `.edu`, `.ac.uk`, `.org`, `.int`) |
| `trusted_seeds` | Domain lists per mind domain — used as documentation and for auto-promotion qualification check |
| `auto_promote_threshold` | Minimum SIGNAL hit count (default 5) for a domain to be proposed for promotion |

**How sources grow week-over-week (automated):**
1. After each run, `recordSourceReputation()` upserts SIGNAL-only findings domains into `source_reputation` table
2. `proposeNewSources()` queries domains with `mention_count ≥ 5`, passes authority TLD check, not on `exclude_always`
3. These appear in `proposed_sources` table (status `pending`) — reviewable in Supabase
4. On next run, `pending` candidates meeting all thresholds auto-promote to the include list
...
### Source routing per mind (current mind names)
| Sources | Minds |
|---------|-------|
| OpenAlex, HN general | All 8 |
| HN insurtech, Federal Register, Reddit | Scout, Null, Weave |
| arXiv climate | Terra |
| arXiv life/longevity | Vita |
| arXiv insurance+ML | Scout, Vita, Terra, Horizon |
| arXiv horizontal (all 5 queries), GitHub, HN security, NIST, dev.to | Horizon, Faro |
| FCA RSS, EIOPA, IAIS | Null, Weave, Horizon, Lex |
| Lloyd's | Terra, Scout |
| SOA | Vita, Faro, Horizon |
| CAS | Scout, Null |
| Geneva Association | Terra, Weave, Faro |
| BIS | Weave, Horizon, Faro |
...
| `source_reputation` | Domains that produced SIGNAL findings — `mention_count` increments weekly; feeds auto-promotion |
| `proposed_sources` | Candidate domains surfaced by `proposeNewSources()` once threshold met — `status: pending/approved/rejected`; run `PROPOSED_SOURCES_SCHEMA.sql` to create |
...
- **Source governance** — `data/sources.json` is the single source of truth for domain allow/exclude lists; loaded at startup by both cron files. `exclude_always` permanently blocks PwC, LinkedIn, consultancy blogs, etc. `verifyRefs()` strips excluded domains even if Claude cites them. Tavily runs with `search_depth:'advanced'` and `exclude_domains` on every call.
- **Source discovery loop** — `recordSourceReputation()` upserts SIGNAL-only domains into `source_reputation`; `proposeNewSources()` surfaces authority-TLD domains with ≥5 hits into `proposed_sources` table for weekly review. Run `PROPOSED_SOURCES_SCHEMA.sql` to enable.
