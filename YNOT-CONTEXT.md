...
-- Signal Trajectories (added 2026-03-30)
...
-- Raw Intelligence Storage (added 2026-03-30)
...
-- Cross-Agent Agreements (added 2026-03-30)
...
-- Monthly Reviews (added 2026-03-30)
...
-- Add regions to findings (added 2026-03-30)
...
| `api/month-review.js` | Monthly narrative synthesis — generates "Month in Review" from 4 weeks of findings + signal trajectories; caches in `monthly_reviews` table |
| `api/visitors.js` | Visitor counter |

### Database (Supabase)
| Table | Purpose |
|-------|---------|
| `findings` | All weekly findings — accumulates over time, never deleted. Now includes `regions` array field |
...
| `signal_trajectories` | Compound signal tracking — topics across weeks with compound_score, trl_velocity, cross_agent_count, trajectory_data (JSONB array of weekly snapshots). Keyed by `topic_key` (unique) |
| `intelligence_raw` | Raw Tavily search results per agent per run — enables re-analysis and source quality tracking |
| `cross_agent_agreements` | Cross-agent topic agreement detection — when 2+ agents independently surface same topic |
...
### What's working well
- **SIGNAL TRAJECTORIES + COMPOUND SCORING (2026-03-30)** — topics that appear across multiple weeks accumulate a compound_score (0-100) based on persistence, confidence, cross-agent agreement, and TRL velocity. Stored in `signal_trajectories` table. Surfaced in frontend Signal Trajectories section and Month in Review.
- **REGIONAL TAGGING (2026-03-30)** — every finding now tagged with `regions` array (US, EU, UK, APAC, Global). Agents assign regions based on geographic context of the finding. Frontend has Regulation and Climate & ESG domain filters.
- **CROSS-AGENT AGREEMENT INDEX (2026-03-30)** — when 2+ agents independently surface the same topic, agreement is detected and stored in `cross_agent_agreements` table. Higher agreement = stronger signal.
...
- Signal History section is still using static historical data (not yet driven by signal_trajectories)
- No search or full-text filter across findings
- New Supabase tables (`signal_trajectories`, `intelligence_raw`, `cross_agent_agreements`, `monthly_reviews`) need to be created in Supabase before first use — code gracefully degrades if missing
- Embedding-based deduplication not yet implemented (currently uses keyword overlap for topic matching)
...
### Vendor-Neutral Rule (NON-NEGOTIABLE — added 2026-03-30)

YNOT.NOW is a **market-level intelligence platform**, not a vendor tracker. This rule is enforced in all agent prompts (`cron.js`, `cron-synthesise.js`, `pulse.js`).

- **NEVER** centre a finding around a single company, consultancy, or vendor (e.g. "Accenture launches...", "McKinsey reports...", "Guidewire releases...")
...
- Pulse/digest bullets follow the same rule: describe market patterns and technology trends, not vendor announcements
...
- ~~Regional intelligence layer: EU vs US vs APAC regulatory divergence~~ ✅ DONE (2026-03-30) — regional tagging on every finding
